...
> R:El programa muestra $2050, así que no coincide.
...
> R:carrito_init inicializa el carrito a un estado vacío conocido: pone cantidad = 0
...
> R:En la función carrito_total, en la línea del for: total += c->items[i].precio; — solo suma el precio unitario de cada producto, sin tener en cuenta la cantidad.

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Debería multiplicar el precio por la cantidad de ese ítem: total += c->items[i].precio * c->items[i].cantidad;
...
> R:El test falla. El 5to carrito_agregar devuelve 1 en vez de 0, cuando el carrito ya debería estar lleno.
...
> R:Sí. Las líneas de la función carrito_descuento (líneas 29-30) tienen #####. No se ejecutaron porque make cobertura solo corre los tests unitarios, y ningún test unitario llama a carrito_descuento, esa función se prueba únicamente en el test de integración (Parte 8), que no entra en esta medición.
...
> R:Un test unitario prueba una función aislada (por ejemplo, solo carrito_total), mientras que uno de integración prueba varias funciones trabajando juntas (agregar productos, calcular total y aplicar descuento en combinación). El bug de carrito_total lo detectó primero un test unitario (test_total_con_cantidad, Parte 5), antes de llegar a los tests de integración.
...
> R:En C no hay control de límites en los arrays, así que escribir fuera de ellos corrompe memoria sin avisar. En Python o Java, eso lanza un error controlado en vez de romper algo silenciosamente.
...
> R:Los tests muestran el error con números concretos y evidencia clara. Mirando el código a simple vista es fácil no notar un detalle como <= en vez de <
...
> R:Con cantidad = 1, sumar el precio una vez da el mismo resultado que multiplicarlo por 1. Por eso ese test no notó que faltaba la multiplicación.
