...
void test_agregar_hasta_llenar(void) {
    ...
    carrito_agregar(&c, p);  /* 4 -> lleno */

    ASSERT_IGUAL(MAX_ITEMS, carrito_contar(&c));  /* deberia contar 4 */

    int resultado = carrito_agregar(&c, p);  /* 5to intento */
    ASSERT_IGUAL(0, resultado);              /* deberia fallar */

    ASSERT_IGUAL(MAX_ITEMS, carrito_contar(&c));  /* el conteo no debe cambiar */
}
